...
from .models import User, Classroom, Quiz, Question, Answer, StudentQuizAttempt, StudentQuestionAttempt, StudentAnswer
...
class ClassroomModelTest(TestCase):
    def setUp(self):
        self.teacher = User.objects.create_user(username='teacher', password='pass', role=User.Role.TEACHER)
...
class QuizModelTest(TestCase):
    ...
    def test_create_quiz(self):
        quiz = Quiz.objects.create(title='Quiz 1', classroom=self.classroom, teacher=self.teacher)
        self.assertEqual(str(quiz), 'Quiz 1')
        self.assertEqual(quiz.question_count(), 0)

    def test_allowed_attempts_validation(self):
        quiz = Quiz(title='Quiz 2', classroom=self.classroom, teacher=self.teacher, allowed_attempts=0)
        with self.assertRaises(ValidationError):
            quiz.clean()
...
class QuestionModelTest(TestCase):
    def setUp(self):
        self.teacher = User.objects.create_user(username='teacher', password='pass', role=User.Role.TEACHER)
        self.classroom = Classroom.objects.create(name='Math', teacher=self.teacher)
        self.quiz = Quiz.objects.create(title='Quiz 1', classroom=self.classroom, teacher=self.teacher)
...
class StudentQuizAttemptModelTest(TestCase):
    def setUp(self):
        ...
        self.quiz = Quiz.objects.create(title='Quiz 1', classroom=self.classroom, teacher=self.teacher)
        self.question = Question.objects.create(quiz=self.quiz, text='Q1')
        self.quiz_attempt = StudentQuizAttempt.objects.create(student=self.student, quiz=self.quiz)
    ...
    def test_calculate_score(self):
        ...
        question_attempt = StudentQuestionAttempt.objects.create(quiz_attempt=self.quiz_attempt, question=self.question)
        question_attempt.submitted_at = timezone.now()
        question_attempt.save()
        StudentAnswer.objects.create(question_attempt=question_attempt, text='A1', is_correct=True)
...
class StudentQuestionAttemptModelTest(TestCase):
    ...
    def test_unique_question_attempt(self):
        StudentQuestionAttempt.objects.create(quiz_attempt=self.quiz_attempt, question=self.question)
        with self.assertRaises(Exception):
            StudentQuestionAttempt.objects.create(quiz_attempt=self.quiz_attempt, question=self.question)

    def test_invalid_question_quiz(self):
        other_quiz = Quiz.objects.create(title='Quiz 2', classroom=self.classroom, teacher=self.teacher)
        other_question = Question.objects.create(quiz=other_quiz, text='Q2')
        attempt = StudentQuestionAttempt(quiz_attempt=self.quiz_attempt, question=other_question)
        with self.assertRaises(ValidationError):
            attempt.clean()
...
class StudentAnswerModelTest(TestCase):
    def setUp(self):
        ...
        self.classroom = Classroom.objects.create(name='Math', teacher=self.teacher)
        self.quiz = Quiz.objects.create(title='Quiz 1', classroom=self.classroom, teacher=self.teacher)
        self.question = Question.objects.create(quiz=self.quiz, text='Q1', time_limit=60)
        self.quiz_attempt = StudentQuizAttempt.objects.create(student=self.student, quiz=self.quiz)
        self.question_attempt = StudentQuestionAttempt.objects.create(quiz_attempt=self.quiz_attempt, question=self.question)
        self.question_attempt.submitted_at = timezone.now()
        self.question_attempt.save()
        Answer.objects.create(question=self.question, text='A1', is_correct=True)

    def test_unique_student_answer(self):
        StudentAnswer.objects.create(question_attempt=self.question_attempt, text='A1', is_correct=True)
        with self.assertRaises(Exception):
            StudentAnswer.objects.create(question_attempt=self.question_attempt, text='A1', is_correct=True)

    def test_clean_methods(self):
        answer = StudentAnswer(question_attempt=self.question_attempt, text='A1', is_correct=True)
        answer.question_attempt.submitted_at = None
        with self.assertRaises(ValidationError):
            answer.clean()

    def test_is_correct_logic(self):
        answer = StudentAnswer.objects.create(question_attempt=self.question_attempt, text='A1')
        self.assertTrue(answer.is_correct)
        answer2 = StudentAnswer.objects.create(question_attempt=self.question_attempt, text='wrong')
